...
export function ShopScreen() {
    ...
    return (
        <ScreenContainer>
            <h1 style="font-size: 28px; margin-bottom: 16px;">🛒 SHOP</h1>
            <GoldDisplay />
                        ...
                        <Card key={weapon.id} style="padding: 12px;">
                            <FlexRow style="gap: 10px;">
                                <div style="flex: 1; min-width: 0;">
                                    <CardTitle style="font-size: 16px; margin-bottom: 6px;">{weapon.name}</CardTitle>
                                    <p style="font-size: 12px; margin: 6px 0; line-height: 1.3;">{weapon.description}</p>
                                    <CardEffect style="font-size: 12px;">Damage: {weapon.damage}</CardEffect>
                                    <p style={`color: ${colors.gold}; font-size: 14px; margin-top: 6px; font-weight: bold;`}>
                                ...
                                <ActionButton
                                    ...
                                    style="padding: 8px 16px; font-size: 14px; min-width: 60px; align-self: flex-start;"
                                >
                                    Buy
                                </ActionButton>
                            </FlexRow>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 12px; margin-bottom: 16px;">
                    {equipment.slice(equipmentStart, equipmentStart + itemsPerPage).map((equip: Equipment) => (
                        <Card key={equip.id} style="padding: 12px;">
                            <FlexRow style="gap: 10px;">
                                <div style="flex: 1; min-width: 0;">
                                    <CardTitle style="font-size: 16px; margin-bottom: 6px;">{equip.name}</CardTitle>
                                    <p style="font-size: 12px; margin: 6px 0; line-height: 1.3;">{equip.description}</p>
                                ...
                                return (
                                    <Card key={dish.id} style="padding: 12px;">
                                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
                                            ...
                                            <CardTitle style="font-size: 16px; margin: 0;">{dish.name}</CardTitle>
                                        </div>
                                        {ingredientIcons && (
                                            <div style="font-size: 16px; margin: 6px 0;">{ingredientIcons}</div>
                                        )}
                                        <p style="font-size: 12px; margin: 6px 0; line-height: 1.3;">{dish.description}</p>
                                        <CardEffect style="font-size: 12px;">{formatDishEffects(dish.effects)}</CardEffect>
                                        <p style="color: #90EE90; font-size: 12px; margin-top: 8px;">Serve in Dining Room or eat for health (Dining Room only)</p>
                                    </Card>
                                );
                            })}
                        </div>
